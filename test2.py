# https://raw.githubusercontent.com/ragoragino/avellaneda-stoikov/master/simulation.py
...
if __name__ == '__main__':
    ...
    for i in np.arange(0, T, dt):
        sym_spread += gamma * sigma**2 * (T - i) + (2/gamma) * np.log(1 + (gamma / k))
    ...
    plt.show()
    #plt.savefig('pnl.pdf', bbox_inches='tight', dpi=100,format='pdf')
    ...
    plt.show()
#    plt.savefig('prices.pdf', bbox_inches='tight', dpi=100,format='pdf')

    print("P&L - Mean of the inventory strategy: {}".format(np.array(pnl_s1).mean()))
    print("P&L - Mean of the symmetric strategy: {}".format(np.array(pnl_s2).mean()))
    print("P&L - Standard deviation of the inventory strategy: {}".format(np.sqrt(np.array(pnl_s1).var())))
    print("P&L - Standard deviation of the symmetric strategy: {}".format(np.sqrt(np.array(pnl_s2).var())))
    print("INV - Mean of the inventory strategy: {}".format(np.array(inventory_s1).mean()))
    print("INV - Mean of the symmetric strategy: {}".format(np.array(inventory_s2).mean()))
    print("INV - Standard deviation of the inventory strategy: {}".format(np.sqrt(np.array(inventory_s1).var())))
    print("INV - Standard deviation of the symmetric strategy: {}".format(np.sqrt(np.array(inventory_s2).var())))
